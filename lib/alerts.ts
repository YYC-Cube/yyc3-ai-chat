...
async function sendSlack(alert: Alert) {
  ...
  if (alert.type === 'error' && env.SLACK_WEBHOOK_URL_ERRORS) {
    ...
    else if (category === 'normal' && env.SLACK_WEBHOOK_URL_NORMAL) webhook = env.SLACK_WEBHOOK_URL_NORMAL
  }
  if (!webhook) return
  ...
  try {
    ...
    const lines = [
      ...
      typeof detail.responseTime === 'number' ? `• RT: ${Math.round(detail.responseTime)}ms` : undefined,
    ].filter(Boolean)
...
async function sendSentry(alert: Alert) {
  ...
  try {
    ...
    if (alert.type === 'error') {
      ...
      Sentry.addBreadcrumb({ category: 'api', message: alert.message, data: extra, level: 'error' })
    } else if (alert.type === 'performance') {
      Sentry.captureMessage(alert.message, 'warning')
      Sentry.addBreadcrumb({ category: 'api', message: alert.message, data: extra, level: 'warning' })
    } else {
