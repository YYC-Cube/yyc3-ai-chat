...
export default function FolderRow({
  ...
  const handleRename = () => {
    const newName = prompt(`${t('message.renameFolder')}:`, name);
    if (newName && newName.trim() && newName !== name) {
      onRenameFolder?.(name, newName.trim());
      toast({
        title: t('button.rename'),
        description: `“${name}” → “${newName.trim()}”`,
      });
    }
    setShowMenu(false);
  };
  ...
  const handleDelete = () => {
    if (confirm(`${t('message.confirmDeleteFolder')} "${name}"?`)) {
      onDeleteFolder?.(name);
      toast({
            ...
            <div className="ml-6 space-y-1 py-1">
              ...
              {conversations.length === 0 && (
                <div className="px-2 py-2 text-xs text-zinc-500 dark:text-zinc-400">
                  {t('message.noConversationsInFolder')}
                </div>
              )}
            </div>
          </motion.div>
