'use client';


import { useState, useEffect } from 'react';


import { useI18n } from '@/lib/i18n';
import { useUserProfile } from '@/lib/user-profile';

export default function SettingsPopover({ children }) {
  const [open, setOpen] = useState(false);
  const [showLanguage, setShowLanguage] = useState(false);
  // 昵称本地状态：绑定到上下文
  const [nickName, setNickName] = useState('');
  const { t, locale, setLocale } = useI18n();
  const { profile, setName } = useUserProfile();

  useEffect(() => {
    setNickName(profile.name || '');
  }, [profile.name]);

  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>{children}</PopoverTrigger>
      <PopoverContent className="w-80 p-0" align="start" side="top">
        <div className="p-4">
          <div className="text-sm text-zinc-600 dark:text-zinc-400 mb-3">j@gmail.com</div>

          <div className="flex items-center gap-3 p-3 rounded-lg bg-zinc-50 dark:bg-zinc-800/50 mb-4">
            <div className="flex items-center gap-2">
              <User className="h-4 w-4" />
              <span className="text-sm font-medium">{t('common.personal')}</span>
            </div>
            <div className="ml-auto">
              <div className="text-xs text-zinc-500">{t('common.proPlan')}</div>
            </div>
            <div className="text-blue-500">
              <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
                <path
                  fillRule="evenodd"
                  d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                  clipRule="evenodd"
                />
              </svg>
            </div>
          </div>

          {/* 昵称设置入口 */}
          <div className="space-y-2 mb-4">
            <div className="text-sm font-medium text-zinc-900 dark:text-zinc-100">
              {t('common.nickname') || 'Nickname'}
            </div>
            <div className="flex items-center gap-2">
              <input
                className="flex-1 rounded-md border border-zinc-200 bg-white px-2 py-1 text-sm dark:border-zinc-700 dark:bg-zinc-900"
                placeholder={t('input.nicknamePlaceholder') || 'e.g. John'}
                value={nickName}
                onChange={(e) => setNickName(e.target.value)}
              />
              <button
                className="rounded-md bg-zinc-900 px-3 py-1 text-xs text-white dark:bg-zinc-100 dark:text-zinc-900"
                onClick={() => {
                  setName(nickName);
                  setOpen(false);
                }}
              >
                {t('common.save') || 'Save'}
              </button>
            </div>
            <div className="text-xs text-zinc-500 dark:text-zinc-400">
              {t('message.template.placeholderHint') || ''}
            </div>
          </div>
          <div className="space-y-1">
            <div className="text-sm font-medium text-zinc-900 dark:text-zinc-100 mb-2">
              {t('common.settings')}
            </div>

            {/* Language */}
            {!showLanguage && (
              <button
                className="flex items-center gap-3 w-full p-2 text-sm text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg"
                onClick={() => setShowLanguage(true)}
              >
                <Globe className="h-4 w-4" />
                <span>{t('common.language')}</span>
                <ChevronRight className="h-4 w-4 ml-auto" />
              </button>
            )}

            {showLanguage && (
              <div className="space-y-1 p-2 border border-zinc-200 rounded-lg dark:border-zinc-700">
                <div className="text-xs text-zinc-500 mb-1">{t('common.language')}</div>
                <button
                  className="flex items-center gap-2 w-full rounded-md px-2 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800"
                  onClick={() => setLocale('zh-CN')}
                >
                  <Check
                    className={`h-4 w-4 ${locale === 'zh-CN' ? 'opacity-100' : 'opacity-0'}`}
                  />
                  <span>{t('common.zhCN')}</span>
                </button>
                <button
                  className="flex items-center gap-2 w-full rounded-md px-2 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800"
                  onClick={() => setLocale('en-US')}
                >
                  <Check
                    className={`h-4 w-4 ${locale === 'en-US' ? 'opacity-100' : 'opacity-0'}`}
                  />
                  <span>{t('common.enUS')}</span>
                </button>
                <div className="text-right">
                  <button
                    className="text-xs text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300"
                    onClick={() => setShowLanguage(false)}
                  >
                    {t('common.close') || 'Close'}
                  </button>
                </div>
              </div>
            )}

            {/* Help */}
            <button className="flex items-center gap-3 w-full p-2 text-sm text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg">
              <HelpCircle className="h-4 w-4" />
              <span>{t('common.getHelp') || 'Get help'}</span>
            </button>

            {/* Upgrade */}
            <button className="flex items-center gap-3 w-full p-2 text-sm text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg">
              <Crown className="h-4 w-4" />
              <span>{t('common.upgradePlan') || 'Upgrade plan'}</span>
            </button>

            {/* Learn more */}
            <button className="flex items-center gap-3 w-full p-2 text-sm text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg">
              <BookOpen className="h-4 w-4" />
              <span>{t('common.learnMore') || 'Learn more'}</span>
              <ChevronRight className="h-4 w-4 ml-auto" />
            </button>

            {/* Logout */}
            <button className="flex items-center gap-3 w-full p-2 text-sm text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg">
              <LogOut className="h-4 w-4" />
              <span>{t('common.logout') || 'Log out'}</span>
            </button>
          </div>
        </div>
      </PopoverContent>
    </Popover>
  );
}
