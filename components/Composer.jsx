...
const Composer = forwardRef(function Composer({ onSend, busy }, ref) {
  ...
  useEffect(() => {
    if (inputRef.current) {
      ...
      const lineHeight = 20; // 估算行高（像素）
      const minHeight = 40;

      // 重置高度以计算滚动高度
      textarea.style.height = 'auto';
      const scrollHeight = textarea.scrollHeight;
      const calculatedLines = Math.max(1, Math.floor((scrollHeight - 16) / lineHeight)); // 16px 为内边距
      ...
      if (calculatedLines <= 12) {
        ...
        // 12+ 行固定高度 + 滚动
        textarea.style.height = `${minHeight + 11 * lineHeight}px`; // 总计 12 行
  ...
  // 个性化占位符：若存在昵称，则按语言显示
  const basePlaceholder = t('input.placeholder');
  const personalizedPlaceholder = nickName
    ? locale === 'zh-CN'
      ? `${nickName}，${basePlaceholder}`
      : `Hi ${nickName}, ${basePlaceholder}`
    : basePlaceholder;
