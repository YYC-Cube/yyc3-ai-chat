  ...
  const handleSubmit = (e) => {
    ...
    if (!folderName.trim()) {
      toast({
        ...
        description: t('message.folderName'),
        variant: 'destructive',
      });
      return;
    }
    try {
      const result = onCreateFolder(folderName.trim());
      if (result && result.ok) {
        toast({
          title: t('button.createFolder'),
        ...
        toast({
          title: t('button.createFolder'),
          description: (result && result.error) || t('message.failed'),
          variant: 'destructive',
        });
      }
    } catch (error) {
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold">{t('message.folderName')}</h2>
              <button
                onClick={handleCancel}
            ...
            <form onSubmit={handleSubmit}>
              <input
                ...
                placeholder={t('input.folderPlaceholder')}
                ...
                autoFocus
                aria-label={t('message.folderName')}
              />
              ...
              <div className="mt-4 flex items-start gap-3 rounded-lg bg-zinc-50 p-4 dark:bg-zinc-800/50">
                ...
                <div className="text-sm text-zinc-600 dark:text-zinc-400">
                  <div className="font-medium mb-1">{t('message.whatsFolder')}</div>
                  <div>{t('message.folderDescription')}</div>
                </div>
              </div>
